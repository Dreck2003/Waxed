...
export const deleteFile = async(req: Req, res: Res,next: Next)=>{
    ...
    if(!nameFile || !courseId) return res.send({error:'The fields are empty',content:null});
    ...
    try{
        ...
        console.log('el path de la imagen es: ',oldFile.url);
        fs.unlink(oldFile.url,(err)=>{
            if(err) return res.status(500).send({error:'not delete',content:null});
